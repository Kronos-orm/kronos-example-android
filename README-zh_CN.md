...
该仓库通过一个完整的 CRUD 应用演示 **Kronos ORM 0.2.4 在 Android 上的使用方式**。示例以 Markdown 笔记本作为业务场景，使用 Android 原生 View、SQLite 和 Markwon 实现。
...
`MarkdownDocument` 是 `KPojo` 数据模型。应用启动时显式注册该模型，并通过以下代码同步 SQLite 表结构：
...
项目内的 `AndroidSQLiteDataSourceWrapper` 将 Android `SQLiteDatabase` 接入 Kronos，所有数据库操作在单线程后台执行器上运行。
...
示例使用 Kotlin `2.4.0`、Android Gradle Plugin `8.13.2`、Kronos `0.2.4` 和 Markwon `4.6.2`。

SQLite 适配器支持 Android 事务块，但不提供 JDBC savepoint、隔离级别或超时参数。
